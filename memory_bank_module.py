...
class MemoryBankMixin(nn.Module):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.register_buffer("_is_fitted_icpa", torch.tensor([False]))

        self.device: torch.device  # defined in lightning module
        self._is_fitted: torch.Tensor
        self._is_fitted_icpa : torch.Tensor
    ...
    def on_validation_start(self) -> None:
        ...
        if self.current_epoch == 0:
            if not self._is_fitted_icpa:
                self.fit()
                self._is_fitted_icpa = torch.tensor([True], device=self.device)
