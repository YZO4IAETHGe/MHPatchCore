...
from IPCA import StreamingIncrementalPCAReductionStrategy
...
class PatchcoreModel(DynamicBufferMixin, nn.Module):
    ...
    def __init__(
        ...
        self._levels: dict[int, list[torch.Tensor]] = {}
        self.ipca = StreamingIncrementalPCAReductionStrategy(0.99)
    ...
    def _farthest_first_coreset(self, data: torch.Tensor, target_size: int) -> torch.Tensor:
        """Select a deterministic farthest-first coreset from a 2D feature array."""
        data_torch = torch.asarray(data, dtype=torch.float32)
        if data_torch.ndim != 2:
            raise ValueError(
                f"Expected 2D feature matrix for coreset selection, got {data_torch.shape}"
            )
        if data_torch.shape[0] == 0:
        ...
        n_samples = int(data_torch.shape[0])
        k = min(int(target_size), n_samples)
        if k == n_samples:
            return data_torch

        mean_vector = torch.mean(data_torch, axis=0, dtype=torch.float32, keepdims=True)
        dist_to_mean = torch.linalg.norm(data_torch - mean_vector, axis=1)
        first_idx = int(torch.argmax(dist_to_mean))

        selected_indices: list[int] = [first_idx]
        min_distances = torch.linalg.norm(data_torch - data_torch[first_idx], axis=1)
        min_distances[first_idx] = 0.0

        #for _ in tqdm.tqdm(range(k-1), desc= "corset_reduction", leave=False):
        while len(selected_indices) < k:
            next_idx = int(torch.argmax(min_distances))
            selected_indices.append(next_idx)
            new_distances = torch.linalg.norm(data_torch - data_torch[next_idx], axis=1)
            min_distances = torch.minimum(min_distances, new_distances)
            min_distances[next_idx] = 0.0

        return data_torch[torch.asarray(selected_indices, dtype=torch.int64)]
    
    def update(
        self,
        batch: torch.Tensor,
    ) -> None:
        batch_torch = torch.asarray(batch, dtype=torch.float32)
        if batch_torch.ndim != 2:
            raise ValueError(
                f"Expected 2D feature batch [N,D], got shape={batch_torch.shape}"
            )
        if batch_torch.shape[0] == 0:
            return
        if not torch.all(torch.isfinite(batch_torch)):
            raise ValueError("StreamingKCenterAggregationStrategy received non-finite features.")

        if self._dimension is None:
            self._dimension = int(batch_torch.shape[1])
        elif int(batch_torch.shape[1]) != int(self._dimension):
            raise ValueError(
                "StreamingKCenterAggregationStrategy received inconsistent "
                f"feature dimensions: expected {self._dimension}, got {batch_torch.shape[1]}."
            )

        self._seen_samples += int(batch_torch.shape[0])
        
        local_size = min(
            self._n_clusters,
            self._chunk_coreset_size,
            int(batch_torch.shape[0]),
        )
        local_block = self._farthest_first_coreset(batch_torch, target_size=local_size)
    ...
    def get_centroids(self) -> torch.Tensor:
        ...
        merged = torch.concatenate(all_blocks, axis=0)
        final_size = min(self._n_clusters, int(merged.shape[0]))
        self.memory_bank = self._farthest_first_coreset(merged, target_size=final_size)
        self._levels = {}

    def _push_merge_reduce_block(self, block: torch.Tensor) -> None:
        candidate = torch.asarray(block, dtype=torch.float32)
        level = 0
        while True:
            ...
            left = slots.pop(0)
            right = slots.pop(0)
            merged = torch.concatenate([left, right], axis=0)
            reduced_size = min(
                self._n_clusters,
                self._chunk_coreset_size,
                int(merged.shape[0]),
            )
            candidate = self._farthest_first_coreset(merged, target_size=reduced_size)
            level += 1